...
class PlayerDetailsView: UIView {
    ...
    var song: Song? {
        didSet
        {
            ...
            minimziedTitleLabel.text = song?.title
            setupNowPlayingInfo()
            setupAudioSession()
            playSong()
        }
    ...
    @objc fileprivate func handlePreviousEpisodeCommand()
    {
        ...
        if let currentEpisodeIndex = playlist.index(where: { $0.title == self.song?.title })
        {
            let previousEpisodeIndex = (currentEpisodeIndex == playlist.count - 1) ? playlist.count - 1 : currentEpisodeIndex - 1
            let previousEpisode = playlist[previousEpisodeIndex]
            self.song = previousEpisode
        }
    }
    ...
    @objc fileprivate func handleNextEpisodeCommand()
    {
        ...
        if let currentEpisodeIndex = playlist.index(where: { $0.title == self.song?.title })
        {
            let nextEpisodeIndex = (currentEpisodeIndex == playlist.count - 1) ? 0 : currentEpisodeIndex + 1
            let nextEpisode = playlist[nextEpisodeIndex]
    ...
    fileprivate func playSong()
    {
        ...
        else if let streamURL = song?.location?.url
        {
